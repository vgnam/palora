"""
PaMaL-QD: PaMaL + Soft Quality-Diversity.
...
class PaMaLQD(PaMaL):
    """PaMaL with Soft Quality-Diversity loss.
    ...
    def configure_model(self, model, data_module=None, keep_original_weights=False) -> nn.Module:
        """Configure model: apply PaMaL subspace + init QD storage."""
        super().configure_model(model, data_module, keep_original_weights)
        model._qd_inputs = []
        logging.info(
            f"PaMaL-QD: QD coeff={self.qd_coefficient}, σ={self.sigma}"
        )
        return model
