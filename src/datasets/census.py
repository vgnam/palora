...
class CensusDataModule(BaseDataModule):
    ...
    def prepare_data(self, *args, **kwargs):
        ...
        self.train = MultiTaskTensorDataset(X_train, y_train, task_names=self.task_names)
        self.valid = MultiTaskTensorDataset(X_valid, y_valid, task_names=self.task_names)
        self.test = MultiTaskTensorDataset(X_test, y_test, task_names=self.task_names)
    ...
    def __download__(root):
        ...
        if not os.path.exists("%s/census-income.data" % str(path)):
            ...
            unzip(
                "%s/census-income.data.gz" % str(path),
                "%s/census-income.data" % str(path),
            )
        ...
        if not os.path.exists("%s/census-income.test" % str(path)):
            ...
            unzip(
                "%s/census-income.test.gz" % str(path),
                "%s/census-income.test" % str(path),
            )
...
def data_Census_preprocessing(
    ...
    X_train = np.concatenate(
        (
            X_continous_train,
            df_training_dummy.values,
            df_training[binary_columns].values,
        ),
        axis=1,
    ).astype("float64")
    X_test = np.concatenate(
        (X_continous_test, df_testing_dummy.values, df_testing[binary_columns].values),
        axis=1,
    ).astype("float64")
