...
class CityScapes(Dataset):
    ...
    def __init__(self, root, train=True, augmentation=False):
        ...
        # read the data file
        if train:
            self.data_path = Path.joinpath(Path(self.root), "train").as_posix()
        else:
            self.data_path = Path.joinpath(Path(self.root), "val").as_posix()
    ...
    def __getitem__(self, index):
        ...
        return image.float(), {"sem": semantic.float(), "depth": depth.float()}
...
class Cityscapes2DataModule(BaseDataModule):
    ...
    def __init__(
        ...
        root=Path.joinpath(Path.home(), "data", "preprocessed", "cityscapes"),
        batch_size=8,
...
class Cityscapes2SplitDataModule(BaseDataModule):
    ...
    def __init__(
        self,
        apply_augmentation=True,
        root=os.path.join(DATA_DIR, "cityscapes"),
        batch_size=8,
        valid_batch_size=None,
        num_workers=10,
        shuffle=True,
