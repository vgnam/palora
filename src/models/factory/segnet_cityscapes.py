...
class _SegNet(nn.Module, BaseCallback):
    """SegNet MTAN"""
    ...
    def forward(self, x, ray: Optional[torch.Tensor] = None):
        ...
        for i in range(5):
            if i == 0:
                g_encoder[i][0] = self.encoder_block[i](x, ray=ray)
                g_encoder[i][1] = self.conv_block_enc[i](g_encoder[i][0], ray=ray)
                g_maxpool[i], indices[i] = self.down_sampling(g_encoder[i][1])
            else:
                g_encoder[i][0] = self.encoder_block[i](g_maxpool[i - 1], ray=ray)
                g_encoder[i][1] = self.conv_block_enc[i](g_encoder[i][0], ray=ray)
                g_maxpool[i], indices[i] = self.down_sampling(g_encoder[i][1])
        ...
        for i in range(5):
            if i == 0:
                ...
                g_decoder[i][0] = self.decoder_block[-i - 1](g_upsampl[i], **ray)
                g_decoder[i][1] = self.conv_block_dec[-i - 1](g_decoder[i][0], **ray)
            else:
        ...
        for i in range(2):
            for j in range(5):
                if j == 0:
                    atten_encoder[i][j][0] = self.encoder_att[i][j](g_encoder[j][0], **ray)
                    atten_encoder[i][j][1] = (atten_encoder[i][j][0]) * g_encoder[j][1]
                    atten_encoder[i][j][2] = self.encoder_block_att[j](atten_encoder[i][j][1], **ray)
            ...
            for j in range(5):
                if j == 0:
                    ...
                    atten_decoder[i][j][0] = self.decoder_block_att[-j - 1](atten_decoder[i][j][0], **ray)
                    atten_decoder[i][j][1] = self.decoder_att[i][-j - 1](
                        torch.cat((g_upsampl[j], atten_decoder[i][j][0]), dim=1),
                        ray=ray,
                    )
        ...
        t1_pred = F.log_softmax(self.pred_task1(atten_decoder[0][-1][-1], **ray), dim=1)
        t2_pred = self.pred_task2(atten_decoder[1][-1][-1], **ray)
        # t3_pred = self.pred_task3(atten_decoder[2][-1][-1])
        # t3_pred = t3_pred / torch.norm(t3_pred, p=2, dim=1, keepdim=True)

        return (
            dict(sem=t1_pred, depth=t2_pred),
            (
                atten_decoder[0][-1][-1],
                atten_decoder[1][-1][-1],
                # atten_decoder[2][-1][-1],
...
class SegNetMtan(nn.Module, BaseCallback):
    def __init__(self, in_channels=3):
        super().__init__()
        self.segnet = _SegNet()
    ...
    def forward(self, x, ray: Optional[torch.Tensor] = None, return_embedding=False):
        if return_embedding:
            return self.segnet(x, ray=ray)
        else:
            pred, rep = self.segnet(x, ray=ray)
            return pred


class SegNetSplitEncoder(nn.Module):
    def __init__(self, model_type="standard", in_channels=3):
        super().__init__()
        # initialise network parameters
    ...
    def forward(self, x, ray: Optional[torch.Tensor] = None):
        if ray is None:
            ...
            ray = dict(ray=ray)  # NOTE: this is a hack to make the code work with the current implementation
        g_encoder, g_decoder, g_maxpool, g_upsampl, indices = ([0] * 5 for _ in range(5))
        for i in range(5):
            g_encoder[i], g_decoder[-i - 1] = ([0] * 2 for _ in range(2))
        ...
        for i in range(5):
            if i == 0:
                g_encoder[i][0] = self.encoder_block[i](x, **ray)
                g_encoder[i][1] = self.conv_block_enc[i](g_encoder[i][0], **ray)
                g_maxpool[i], indices[i] = self.down_sampling(g_encoder[i][1])
            else:
                g_encoder[i][0] = self.encoder_block[i](g_maxpool[i - 1], **ray)
                g_encoder[i][1] = self.conv_block_enc[i](g_encoder[i][0], **ray)
                g_maxpool[i], indices[i] = self.down_sampling(g_encoder[i][1])
        ...
        # return [t1_pred, t2_pred], g_decoder[i][1]  # NOTE: last element is representation
        return g_decoder[i][1]  # NOTE: last element is representation
...
class SegNetSegmentationDecoder(nn.Module):
    ...
    def forward(self, x, ray: Optional[torch.Tensor] = None):
        ray = dict(ray=ray) if ray is not None else {}
        t1_pred = F.log_softmax(self.pred_task1(x, **ray), dim=1)
        return t1_pred
...
class SegNetDepthDecoder(nn.Module):
    def __init__(self, filters=FILTER) -> None:
        ...
        self.pred_task2 = nn.Sequential(
            ...
            nn.Conv2d(in_channels=filters[0], out_channels=1, kernel_size=1, padding=0),
        )

    def forward(self, x, ray: Optional[torch.Tensor] = None):
        ray = dict(ray=ray) if ray is not None else {}
        t2_pred = self.pred_task2(x, **ray)
        return t2_pred


class SegNet(nn.Module, BaseCallback):
    def __init__(self, in_channels=3):
        super().__init__()
        self.segnet = SegNetSplitEncoder(in_channels=in_channels)
        self.segmentation_decoder = SegNetSegmentationDecoder(filters=FILTER)
        self.depth_decoder = SegNetDepthDecoder(filters=FILTER)
    ...
    def forward(self, x, ray: Optional[torch.Tensor] = None, return_embedding=False):
        emb = self.segnet(x, ray=ray)
        ray = dict(ray=ray) if ray is not None else {}

        t1 = self.segmentation_decoder(emb, **ray)
        t2 = self.depth_decoder(emb, **ray)
        result = [t1, t2]
        result = {"sem": t1, "depth": t2}
        if return_embedding:
            return result, emb
        else:
            return result
