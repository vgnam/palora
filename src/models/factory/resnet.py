...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, ray=None):
        ...
        out = F.relu(self.bn1(self.conv1(x, **ray)))
        out = self.bn2(self.conv2(out, **ray))
        out += self.shortcut(x, **ray)
        out = F.relu(out)
        return out
...
class ResNetEncoder(nn.Module):
    ...
    def forward(self, x, ray=None):
        if ray is None:
            ...
            ray = dict(ray=ray)  # NOTE: this is a hack to make the code work with the current implementation
        out = F.relu(self.bn1(self.conv1(x, **ray)))
        out = self.layer1(out, **ray)
        out = self.layer2(out, **ray)
        out = self.layer3(out, **ray)
        out = self.layer4(out, **ray)
...
class CelebAResnet(SharedBottom):
    def __init__(self, task_names):
        ...
        super().__init__(encoder=encoder, decoder=decoder, task_names=task_names)
...
class UtkFaceResnet(SharedBottom):
    def __init__(self, in_channels=3):
        ...
        decoder = [
            MLPDecoder(num_classes=1),
            MLPDecoder(num_classes=2),
            MLPDecoder(num_classes=5),
        ]
        task_names = ["age", "gender", "race"]
        decoder = dict(zip(task_names, decoder))
        super().__init__(encoder=encoder, decoder=decoder, task_names=task_names)
