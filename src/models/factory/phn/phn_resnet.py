...
class ResnetHyper(nn.Module):
    def __init__(
        ...
        self.layer_to_shape = {
            "resnet.conv1.weight": torch.Size([64, 3, 3, 3]),  # torch.Size([64, 3, 7, 7]),
            ...
            "task1.weight": torch.Size([1, 2048]),
            "task1.bias": torch.Size([1]),
            "task2.weight": torch.Size([2, 2048]),
            "task2.bias": torch.Size([2]),
            "task3.weight": torch.Size([5, 2048]),
            "task3.bias": torch.Size([5]),
        }
    ...
    def forward(self, preference):
        ...
        for i, pref in enumerate(preference):
            pref_embedding += (
                self.preference_embedding_matrix(torch.tensor([i], device=preference.device)).squeeze(0) * pref
            )
        # chunk embedding
        weights = []
        for chunk_id in range(self.num_chunks):
            chunk_embedding = self.chunk_embedding_matrix(torch.tensor([chunk_id], device=preference.device)).squeeze(
                0
            )
            # input to fc
...
class ResNetTarget(nn.Module):
    def __init__(self, pretrained=False, progress=True, **kwargs):
        super().__init__()
        self.resnet = torchvision.models.resnet18(pretrained=pretrained, progress=progress, num_classes=512, **kwargs)

        # self.resnet.conv1.weight.data = torch.randn((64, 1, 7, 7))

        self.task1 = nn.Linear(2048, 1)
        self.task2 = nn.Linear(2048, 2)
        self.task3 = nn.Linear(2048, 5)
    ...
    def forward(self, x, weights=None):
        ...
        if weights is None:
            x = self.resnet(x)
            x = F.relu(x)
            p1, p2, p3 = self.task1(x), self.task2(x), self.task3(x)
            return p1, p2, p3
            ...
            p3 = self.forward_clf(x, weights, 3)
    ...
    def forward_init(x, weights):
        ...
        x = F.conv2d(x, weights["resnet.conv1.weight"], stride=1, padding=1)
        x = F.batch_norm(
            x,
            torch.zeros(x.data.size()[1]).to(device),
            torch.ones(x.data.size()[1]).to(device),
