...
class EPOSolver(Solver):
    ...
    def __init__(self, num_tasks, n_params):
        super().__init__(num_tasks)
        self.solver = EPO(num_tasks=num_tasks, n_params=n_params)
...
class EPO:
    ...
    def get_weighted_loss(self, losses, ray, parameters):
        lp = ExactParetoLP(m=self.num_tasks, n=self.n_params, r=ray.cpu().numpy())
